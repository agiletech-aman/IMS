...
Route::middleware(EnsureStaticAuthenticated::class)->group(function () {
    ...
    Route::prefix('asset-management')->name('asset-management.')->group(function () {
        ...
        foreach ([
            ...
            Route::match(['put', 'patch'], "{$path}/{record}", [AssetMasterController::class, 'update'])->middleware($permission($module, 'update'))->name($path.'.update');
            Route::delete("{$path}/{record}", [AssetMasterController::class, 'destroy'])->middleware($permission($module, 'delete'))->name($path.'.destroy');
        }
    });
