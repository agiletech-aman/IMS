    ...
    <div class="modal-dialog modal-dialog-centered"><div class="modal-content panel">
        ...
        <form method="POST" action="{{ $action }}" enctype="multipart/form-data">@csrf @if($editing) @method('PUT') @endif
            <div class="modal-body p-4"><div class="row g-3">
                <div class="col-md-7"><label class="form-label">Name <span class="text-danger">*</span></label><input class="form-control" name="name" value="{{ $value('name') }}" placeholder="Enter {{ strtolower($singular) }} name" required></div>
                ...
                @elseif($module === 'sub-types')
                    <div class="col-12"><label class="form-label">Asset Type <span class="text-danger">*</span></label><select class="form-select" name="asset_type_id" required><option value="">Select asset type</option>@foreach($options['asset_type_id'] as $id=>$label)<option value="{{ $id }}" @selected((string)$value('asset_type_id') === (string)$id)>{{ $label }}</option>@endforeach</select></div>
                    <div class="col-12"><div class="form-check mt-1"><input class="form-check-input" type="checkbox" name="is_required" value="1" @checked((bool) old('is_required', $editing ? $record->is_required : true))><label class="form-check-label">Required field in asset forms and imports</label></div></div>
